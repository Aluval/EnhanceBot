...
async def enhance_video(client: Client, message: Message):
    if not message.reply_to_message or not message.reply_to_message.video:
        return await message.reply("âŒ Please reply to a video file with /enhance.")

    video_msg = message.reply_to_message
    file_size = video_msg.video.file_size

    if file_size > MAX_FILE_SIZE:
        return await message.reply("âŒ File is larger than 300MB. Please send a smaller video.")

    start = time.time()
    downloading = await message.reply("â¬‡ï¸ Downloading video...")
    ...
    if not os.path.exists(input_path) or os.path.getsize(input_path) == 0:
        return await message.reply("âŒ Download failed or file is empty.")
    ...
    try:
        ...
        return await message.reply(f"âŒ Couldn't get video duration: {e}")

    output_path = "enhanced.mp4"
    processing_msg = await message.reply("âš™ï¸ Enhancing video...")
    ...
    while True:
        ...
        if match:
            ...
            if percent != last_percent and percent > 0:
                ...
                await processing_msg.edit_text(f"âš¡ Enhancing video: {percent}%\nETA: {eta_formatted}")
    ...
    if retcode != 0:
        await message.reply(f"âŒ FFmpeg failed with code {retcode}.")
        os.remove(input_path)
        return

    if not os.path.exists(output_path) or os.path.getsize(output_path) == 0:
        await message.reply("âŒ Enhanced file is empty or missing.")
        os.remove(input_path)
        return

    upload_msg = await message.reply("â¬†ï¸ Uploading enhanced video...")
    await client.send_chat_action(message.chat.id, ChatAction.UPLOAD_VIDEO)

    await message.reply_video(
        video=output_path,
        caption="âœ… Enhanced Video (1080p) with original audio and subtitles",
        progress=progress,
